-- Demo seed: analytics-ready equipment and a mixed-demand volatility forecast.
-- Dataset: exactly 300 idempotent demo reservations from 2025-05-25 through 2026-12-23.
-- Narrative: mixed peaks and troughs across both comparison windows, with additional
--            off-window surges to test charts that should not infer a simple trend.
-- Demand profile:
--   * 2025-05-25 to 2025-06-03:  25 reservations (surge)
--   * 2025-06-04 to 2025-06-13:  12 reservations (dip)
--   * 2025-06-14 to 2025-06-23:  23 reservations (surge)
--     Prior-year May 25 to June 23 total: 60 reservations (mixed high/low/high)
--   * 2025-06-24 to 2025-08-31:  21 reservations (quiet baseline)
--   * 2025-09-01 to 2025-10-15:  20 reservations (moderate)
--   * 2025-10-16 to 2025-12-31:  24 reservations (secondary seasonal surge)
--   * 2026-01-01 to 2026-03-31:  28 reservations (baseline)
--   * 2026-04-01 to 2026-05-24:  18 reservations (moderate)
--   * 2026-05-25 to 2026-06-03:  28 reservations (surge)
--   * 2026-06-04 to 2026-06-12:   8 reservations (dip)
--   * 2026-06-13 to 2026-06-23:  24 reservations (surge)
--     Current-term May 25 to June 23 total: 60 reservations (mixed high/low/high)
--   * 2026-06-24 to 2026-08-31:  24 reservations (cool-down)
--   * 2026-09-01 to 2026-10-31:  20 reservations (secondary surge)
--   * 2026-11-01 to 2026-12-23:  25 reservations (year-end baseline)
--
...
--   Get-Content -Raw docs/demo-seeds/Mixed.sql | docker compose -f compose.dev.yml exec -T database psql -U techreserve_user -d techreserve
--
...
periods AS (
    ...
    FROM (
        VALUES
            (1,  DATE '2025-05-25', DATE '2025-06-03', 25, 'surge'),
            (2,  DATE '2025-06-04', DATE '2025-06-13', 12, 'dip'),
            (3,  DATE '2025-06-14', DATE '2025-06-23', 23, 'surge'),
            (4,  DATE '2025-06-24', DATE '2025-08-31', 21, 'baseline'),
            (5,  DATE '2025-09-01', DATE '2025-10-15', 20, 'moderate'),
            (6,  DATE '2025-10-16', DATE '2025-12-31', 24, 'surge'),
            (7,  DATE '2026-01-01', DATE '2026-03-31', 28, 'baseline'),
            (8,  DATE '2026-04-01', DATE '2026-05-24', 18, 'moderate'),
            (9,  DATE '2026-05-25', DATE '2026-06-03', 28, 'surge'),
            (10, DATE '2026-06-04', DATE '2026-06-12',  8, 'dip'),
            (11, DATE '2026-06-13', DATE '2026-06-23', 24, 'surge'),
            (12, DATE '2026-06-24', DATE '2026-08-31', 24, 'baseline'),
            (13, DATE '2026-09-01', DATE '2026-10-31', 20, 'surge'),
            (14, DATE '2026-11-01', DATE '2026-12-23', 25, 'baseline')
...
business_days AS (
    ...
    WHERE EXTRACT(ISODOW FROM generated_day.calendar_day) <= 6
        -- Include the requested 2025-05-25 boundary even though it is a Sunday;
        -- online reservation submissions can legitimately occur outside office hours.
        OR (
            periods.period_id = 1
            AND generated_day.calendar_day::date = periods.start_date
        )
),
...
reservation_slots AS (
    SELECT
        ...
        CASE
            -- Pin one online submission to the requested first date and one to
            -- the final date; all other reservations remain scattered.
            WHEN periods.period_id = 1
                AND generated_slot.slot_no = 1
                THEN 1
            WHEN periods.period_id = 14
                AND generated_slot.slot_no = periods.reservation_count
                THEN business_day_counts.day_count
            ELSE 1 + MOD(
...
selected_kit_catalog AS (
    SELECT
        'low'::text AS kit_scale,
        kit_id,
        organization_name,
        borrower_slot,
        activity_type,
        ...
        requested_equipment_list,
        requested_quantity
    FROM low_kit_catalog
    UNION ALL
    SELECT
        'high'::text AS kit_scale,
        kit_id,
        organization_name,
        borrower_slot,
        activity_type,
...
enriched_reservations AS (
    ...
    JOIN selected_kit_catalog AS kit_catalog
        ON kit_catalog.kit_scale = CASE
            WHEN scattered_reservations.demand_band = 'surge' THEN 'high'
            ELSE 'low'
        END
        AND kit_catalog.kit_id = CASE
            WHEN scattered_reservations.demand_band = 'surge'
                THEN 1 + MOD(scattered_reservations.seed_sequence * 7, 16)
            ELSE
...
statused_reservations AS (
    SELECT
        ...
        CASE
            -- Current in-window surge retains some pending requests for operational stress.
            WHEN demand_band = 'surge'
                AND submission_date BETWEEN DATE '2026-06-13' AND DATE '2026-06-23'
                AND MOD(seed_sequence, 4) = 0 THEN 'Pending Review'
            -- Future reservations are mostly approved/pending rather than completed.
            WHEN submission_date > DATE '2026-06-25'
                AND MOD(seed_sequence, 7) = 0 THEN 'Pending Review'
            -- Rejections occur across all bands, but slightly more often at surge points.
            WHEN demand_band = 'surge'
                AND MOD(seed_sequence, 19) = 0 THEN 'Rejected'
            WHEN demand_band IN ('dip', 'baseline')
                AND MOD(seed_sequence, 29) = 0 THEN 'Rejected'
            -- Past surge activity has a realistic mix of closed and approved reservations.
            WHEN demand_band = 'surge'
                AND submission_date < DATE '2026-06-25'
                AND MOD(seed_sequence, 5) = 0 THEN 'Completed'
            WHEN demand_band = 'surge' THEN 'Approved'
        ...
        CASE
            WHEN demand_band = 'surge'
                AND (requested_quantity >= 7 OR MOD(seed_sequence, 4) = 0) THEN 'High'
            WHEN demand_band = 'surge' THEN 'Normal'
            WHEN demand_band = 'moderate' AND requested_quantity >= 4 THEN 'Normal'
            WHEN demand_band = 'dip' OR requested_quantity <= 2 OR MOD(seed_sequence, 7) = 0 THEN 'Low'
            ELSE 'Normal'
...
-- Validation: exactly 300 records, 60 mixed reservations in each comparison
-- window, a non-zero low-demand dip in both windows, and records at both requested boundaries.
SELECT
    COUNT(*) AS total_demo_reservations,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2025-05-25' AND DATE '2025-06-23'
    ) AS prior_year_mixed_window,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2026-05-25' AND DATE '2026-06-23'
    ) AS current_term_mixed_window,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2025-06-04' AND DATE '2025-06-13'
    ) AS prior_year_dip,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2026-06-04' AND DATE '2026-06-12'
    ) AS current_term_dip,
    COALESCE(SUM(requested_quantity) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2025-05-25' AND DATE '2025-06-23'
    ), 0) AS prior_year_requested_units,
    COALESCE(SUM(requested_quantity) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2026-05-25' AND DATE '2026-06-23'
    ), 0) AS current_term_requested_units,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date BETWEEN DATE '2025-05-25' AND DATE '2026-12-23'
    ) AS in_requested_date_range,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date = DATE '2025-05-25'
    ) AS reaches_requested_start_date,
    COUNT(*) FILTER (
        WHERE submission_timestamp::date = DATE '2026-12-23'
    ) AS reaches_requested_end_date
FROM reservations
...
SELECT
    ...
    COUNT(*) FILTER (WHERE priority_level = 'High') AS high_priority_reservations
FROM reservations
WHERE reservation_code LIKE 'DEMO-2526-%'
