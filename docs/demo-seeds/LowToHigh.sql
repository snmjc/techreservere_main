-- Demo seed: analytics-ready equipment and a two-period recovery forecast.
-- Dataset: exactly 300 idempotent demo reservations from 2025-05-25 through 2026-12-23.
-- Narrative: low demand last year, high demand this semester.
-- Demand profile:
...
--   * 2025-09-01 to 2025-12-31:  50 reservations (steady baseline)
--   * 2026-01-01 to 2026-05-24:  48 reservations (steady baseline)
--   * 2026-05-25 to 2026-06-23: 105 reservations (current-term recovery spike)
--   * 2026-06-24 to 2026-08-31:  22 reservations (post-spike cool-down)
--   * 2026-09-01 to 2026-12-23:  23 reservations (quiet year-end baseline)
--
...
--   Get-Content -Raw docs/demo-seeds/LowToHigh.sql | docker compose -f compose.dev.yml exec -T database psql -U techreserve_user -d techreserve
--
...
periods AS (
    ...
    FROM (
        VALUES
            ...
            (3, DATE '2025-09-01', DATE '2025-12-31', 50, 'baseline'),
            (4, DATE '2026-01-01', DATE '2026-05-24', 48, 'baseline'),
            (5, DATE '2026-05-25', DATE '2026-06-23', 105, 'current_peak'),
            (6, DATE '2026-06-24', DATE '2026-08-31', 22, 'cool_down'),
            (7, DATE '2026-09-01', DATE '2026-12-23', 23, 'baseline')
...
reservation_slots AS (
    SELECT
        ...
        CASE
            -- Pin one record to the requested final date so the fixture reaches
            -- 2026-12-23 while the other reservations remain scattered.
            WHEN periods.period_id = 7
...
enriched_reservations AS (
    ...
    JOIN selected_kit_catalog AS kit_catalog
        ON kit_catalog.kit_scale = CASE
            WHEN scattered_reservations.demand_band = 'current_peak' THEN 'high'
            ELSE 'low'
        END
    ...
    SELECT
        ...
        CASE
            WHEN demand_band = 'current_peak'
                AND submission_date >= DATE '2026-06-16'
            ...
            WHEN demand_band = 'current_peak' THEN 'Approved'
        ...
        CASE
            ...
            WHEN demand_band = 'current_peak' THEN 'Normal'
...
-- Validation: this must report exactly 300 demo reservations, with 15 in the
-- prior-year low-demand window and 105 in the current-term recovery spike.
SELECT
