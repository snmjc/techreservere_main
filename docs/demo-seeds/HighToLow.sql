-- Demo seed: analytics-ready equipment and reservation demand forecast.
-- Dataset: exactly 300 idempotent demo reservations from 2025-05-25 through 2026-06-23.
-- Narrative: seasonal spike in the prior year, softer current term.
-- Demand profile:
--   * 2025-05-25 to 2025-06-23: 105 reservations (prior-year high demand)
--   * 2025-06-24 to 2025-08-31:  18 reservations (quiet break)
--   * 2025-09-01 to 2025-12-31:  72 reservations (steady term)
--   * 2026-01-01 to 2026-04-30:  72 reservations (steady term)
--   * 2026-05-25 to 2026-06-23:  33 reservations (softer current term)
--
-- Run from the repo root while Docker is running:
--   Get-Content -Raw docs/demo-seeds/HighToLow.sql | docker compose -f compose.dev.yml exec -T database psql -U techreserve_user -d techreserve
--
...
periods AS (
    ...
    FROM (
        VALUES
            ...
            (2, DATE '2025-06-24', DATE '2025-08-31', 18, 'quiet_break'),
            (3, DATE '2025-09-01', DATE '2025-12-31', 72, 'steady_term'),
            (4, DATE '2026-01-01', DATE '2026-04-30', 72, 'steady_term'),
            (5, DATE '2026-05-25', DATE '2026-06-23', 33, 'current_soft')
...
enriched_reservations AS (
    ...
    JOIN kit_catalog
        ON kit_catalog.kit_id = CASE
            ...
            WHEN scattered_reservations.demand_band = 'current_soft'
                THEN 9 + MOD(scattered_reservations.seed_sequence * 7, 8)
            ELSE
...
-- and 33 during the softer current-term comparison window.
SELECT
