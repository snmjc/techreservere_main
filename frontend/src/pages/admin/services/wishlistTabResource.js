...
const WISHLIST_TAB_CACHE_PREFIX = 'techreserve_wishlist_tab_v1';
const DEFAULT_CACHE_TTL_MS = 5 * 60 * 1000;

export function useWishlistTabResource({
  tabKey,
  label,
  cacheScope,
  fetchAccounts,
  ...
  async function load() {
    ...
    try {
      ...
      error.value = loadError?.message || 'Unable to load wishlist accounts.';
...
function writeTabCache(tabKey, cacheScope, data, cacheTtlMs) {
  ...
  try {
    const nextExpiresAt = Date.now() + cacheTtlMs;
...
function cacheKey(tabKey, cacheScope) {
  return `${WISHLIST_TAB_CACHE_PREFIX}_${cacheScope || 'anonymous'}_${tabKey}`;
}
